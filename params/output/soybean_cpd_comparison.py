...
import numpy as np
from output_utils import parse_text_list, scale_from_matrix
import argparse
...
def compute_metrics(A, t, xy_err, yaw_err, xy_scl_err):
    ...
    return rot_err, trans_err, scl_err, xy_err_mag, yaw_err, xy_scl_err_mag
...
def main():
    ...
    for entry in input_list:
        ...
        result.append(compute_metrics(A, t, xy_err, yaw_err, xy_scl_err))

    result = np.vstack(result) if result else np.zeros((0, 6), dtype=float)
    np.savetxt(results_folder / "soybean_row3_cpd_comparison_result.csv", result, delimiter=',', header='rot_err,trans_err,scl_err,xy_err_mag,yaw_err,xy_scl_err_mag', comments='')
    np.save(results_folder / "soybean_row3_cpd_comparison_result.npy", result)
